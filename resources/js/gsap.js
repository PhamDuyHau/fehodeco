...
if (import.meta.hot) {
  import.meta.hot.dispose(() => {
    window.__GSAP_INIT__ = false;
    ScrollTrigger.getAll().forEach((st) => st.kill());
  });
}
if (!window.__GSAP_INIT__) {
  window.__GSAP_INIT__ = true;
  ...
  function initHero() {
    ...
    tl.from(heroImage, { scale: 1.12, duration: 1.8 })
      .from(
        ".hero-hodeco span",
        { opacity: 0, duration: 1, stagger: 0.03 },
        "-=0.6",
      )
  ...
  function initHoverEffects() {
    ...
    const corners = gsap.utils.toArray(
      ".home-card-corner, .cutout-corner, .card-corner",
    );
...
function initRoadAnimation() {
  ...
  const allYears = [
    { year: '2015', text: 'Vốn điều lệ 411 tỷ đồng',              img: 'blue',   end: 0.23 },
    { year: '2019', text: 'Huân chương lao động hạng I.<br>Vốn điều lệ 580 tỷ đồng', img: 'orange', end: 0.58 },
    { year: '2022', text: 'Vốn điều lệ 1.080 tỷ đồng',            img: 'blue',   end: 0.76 },
    { year: '2026', text: 'Vốn điều lệ 1.998 tỷ đồng',            img: 'orange', end: 0.86 },
    { year: '1991', text: 'Thành lập công ty',                     img: 'blue',   end: 0.23 },
    { year: '1995', text: 'Vốn điều lệ 50 tỷ đồng',               img: 'orange', end: 0.38 },
    { year: '2000', text: 'Vốn điều lệ 120 tỷ đồng',              img: 'blue',   end: 0.58 },
    { year: '2008', text: 'Vốn điều lệ 280 tỷ đồng',              img: 'orange', end: 0.76 },
  ];
  ...
  const slots  = [0,1,2,3].map(i => document.querySelector(`#dot-slot-${i}`));
  const labels = [0,1,2,3].map(i => document.querySelector(`#label-slot-${i}`));
  const btnNext = document.querySelector('#road-next');
  const btnPrev = document.querySelector('#road-prev');

  if (!slots[0] || !labels[0]) return;

  const endPositions = [0.23, 0.58, 0.76, 0.86];
  ...
  function playPage(page) {
    ...
    gsap.to([...slots, ...labels], {
      ...
      onComplete() {
        ...
        // swap content + reset dots to path start
        ...
        years.forEach((data, i) => {
          gsap.timeline({ delay: i * 0.15 })
            ...
            .call(() => { if (i === years.length - 1) isAnimating = false; });
        });
  ...
  btnNext.addEventListener('click', () => {
    if (currentPage < totalPages - 1) { currentPage++; playPage(currentPage); }
  });
  btnPrev.addEventListener('click', () => {
    if (currentPage > 0) { currentPage--; playPage(currentPage); }
  });
